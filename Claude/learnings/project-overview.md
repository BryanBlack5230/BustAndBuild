...
> ⚠️ **Superseded by [ADR-0005](../docs/adr/0005-ecs-namespaced-under-battle.md)** — ECS moves out of the global namespace into `Gameplay.Battle` (shared core = un-foldered root) + nested feature namespaces (`Battle.{Combat,Throwing,Structures,Economy,Feedback,Spawning,Director,Bridge,AI}`). The paragraph below is the **legacy / pre-migration** state — still true of unmigrated files; new ECS files follow ADR-0005.
