...
## Pattern — Burst-Friendly Static ECB Spawn Helper (PearlSpawnUtility)
A plain `static class` method **is callable from inside Burst-compiled `IJobEntity.Execute`** as long as it touches only unmanaged types. `PearlSpawnUtility.Spawn(ref EntityCommandBuffer.ParallelWriter ecb, int sortKey, Entity prefab, in PearlSettings settings, float3 source, int count, float valuePerPearl, ref Random rand)` deduplicates the ~30-line pearl instantiate-and-configure block previously copy-pasted between `SpawnPearlsOnDeathJob` and `EnemyEscapeJob`. Notes: pass the job's ECB field by `ref` (it's a struct), pass `Random` by `ref` so state advances in the caller, take config structs by `in`. Prefer carrying one settings struct field on the job (`public PearlSettings Settings;`) over copying individual floats. Lives in root `Systems/` (global namespace) because both global-namespace and `Gameplay.AI` jobs call it.
