...
- **Drop table = per-enemy buffer.** `EnemyAuthoring` serializes a `List<DropTableEntry>` (`type, min, max, chance, value`), baked into a `DynamicBuffer<ResourceDrop>`. Each row is rolled **independently** on death (pearls = a 100%-chance row; wood = 0.20; etc.) — an enemy can drop several resources at once. Authoring defaults to one pearl row so existing enemies keep dropping pearls. `DropTableEntry` is a standalone serializable struct so it lifts into the per-unit-type config SO when the [[ConfigTask]] hub lands.
