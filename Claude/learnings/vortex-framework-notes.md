...
| `Unity/UI/PoolSystem/Pool(+Item).cs` | `Infrastructure/Pooling/UiPool(+Item).cs` | Dropped Vortex `TimeController`/`IDataStorage`/Odin deps and LINQ; **dropped the `params object[]` key API** (array-instance identity was a removal footgun) — key is a single data object now; missing prefab throws instead of logging |

`[Formula]` usage: `[Formula(nameof(_slots))] [SerializeField] string _wealthFormula; [SerializeField] FormulaSlot[] _slots;` — slots bind `{i}` tokens to numeric members of the declaring object; runtime eval via `FormulaEvaluator.Calc(formula, slots, owner)` (reflection, cached accessors — config-build time, not per-frame). Parser supports `+ - * / ^`, parens, `pi`/`e`, sqrt/abs/sin/cos/tan/log/floor/ceil/round/min/max/pow/clamp. Target use: Wealth/Danger formulas, economy curves (TaskBreakdown §12).
