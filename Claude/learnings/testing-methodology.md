...
## Don't swap the default world — pass the EntityManager in (ADR-0008)
A Mono↔ECS class that grabs `World.DefaultGameObjectInjectionWorld.EntityManager` in its ctor forces its integration tests to mutate a process-global: `_previousDefault = World.DefaultGameObjectInjectionWorld;` → `World.DefaultGameObjectInjectionWorld = new World("…Test")` in `[SetUp]`, restore in `[TearDown]`. That's ordering-sensitive (the restore must always run), leaks into every test, and was the smell behind ADR-0008. **Resolved idiom:** the class takes the `EntityManager` via `IWorldInitializable.Initialize(em)` ([[di-architecture]]), so the test builds a throwaway `World` and calls `sut.Initialize(_world.EntityManager)` after constructing it — no global touch, matching the **AI suite** bar (`Tests/AI/*`). `PlacementControllerTests`, `BeaconEcsBridgeTests`, `BeaconCoreInsertTests` were migrated this way; `BeaconInvulnerabilityTests` drives `ApplyDamageSystem` directly and never swapped. When the SUT subscribes to the static `EventBus` inside `Initialize`, still `Dispose()` it in `[TearDown]` (domain reload is OFF — a leaked handler fires into a later test).
