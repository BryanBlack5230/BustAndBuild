...
public sealed class SceneWorkflowRunner : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
        var configPath = UnityEditor.SessionState.GetString(EditorConstants.SceneWorkflow.RunConfigPathKey, string.Empty);
        if (string.IsNullOrEmpty(configPath)) return;

        var runConfig = UnityEditor.AssetDatabase.LoadAssetAtPath<RunConfiguration>(configPath);
        if (runConfig == null) return;

        var isSingleScene = UnityEditor.SessionState.GetBool(EditorConstants.SceneWorkflow.IsSingleSceneKey, false);
        var singleSceneTarget = UnityEditor.SessionState.GetString(EditorConstants.SceneWorkflow.SingleSceneTargetKey, string.Empty);
