...
> Bust and Build is confirmed Force Text (`m_SerializationMode: 2`). A file that is not `%YAML 1.1` here is almost certainly an Odin-binary asset — do not hand-edit it (see SKILL.md → Project specifics).
...
> Note for this project: `[field: SerializeField]` is rarely used here — the convention is `_camelCase` private `[SerializeField]` fields, which serialize under their plain name (`_cost:`).
...
1. Save the file.
2. Focus the Unity Editor so it re-imports the asset (`AssetDatabase.Refresh()`).
...
→ The project or this specific asset is in Binary or Mixed serialization mode (or, in this project, an Odin-binary asset). Do not hand-edit. Open it in the Unity Editor instead.
