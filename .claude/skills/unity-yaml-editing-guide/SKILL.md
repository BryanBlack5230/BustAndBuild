...
description: >-
  Provides guidelines for directly editing Unity YAML-serialized asset files for this project.
...
license: Unlicense
metadata:
  author: Koji Hasegawa (adapted for BarkingBird studio)
---

Guide for directly editing Unity YAML-serialized asset files for this project.
...
- **Never directly edit `.unity` or `.prefab` files.** Those carry GameObject/Prefab-instance structure that is unsafe to author by hand — open them in the Unity Editor (this project has no scripted scene-edit / MCP route configured).
- **Never touch `.meta` files.** Unity owns `.meta` content (GUIDs, importer settings); hand-edits corrupt asset references project-wide. (`Write(*.meta)` is denied in settings.)
...
- **After editing, let Unity re-import the asset** (focus the Editor) and confirm the value survived by reviewing the diff Unity produces on next save.

## Gotchas

- **Verify the file starts with `%YAML 1.1` before editing.** If it does not, the asset must not be hand-edited — open it in the Unity Editor instead.
...
## Project specifics (Bust and Build)

- **Serialization is Force Text** (`ProjectSettings/EditorSettings.asset` → `m_SerializationMode: 2`, verified) — `.asset`/`.mat` files are valid text YAML. If any file does **not** start with `%YAML 1.1`, stop — it is Odin-binary or predates the setting (see below).
- **Odin-serialized data is NOT hand-editable as plain YAML.** This project uses Odin Inspector (Sirenix). Anything stored *through Odin* — `[OdinSerialize]` fields, `SerializedScriptableObject` / `SerializedMonoBehaviour` subclasses, or types Unity itself can't serialize — lands inside a `serializationData` blob (base64 `SerializedBytes` + a `referencedUnityObjects` list), **not** as readable field lines. Edit those via the Inspector (or the `odin-visual-designer` skill for layout), never by hand. Plain `[SerializeField]` fields on the same asset are still normal YAML and fine to edit.
- **Field naming:** the convention here is `_camelCase` private `[SerializeField]` fields (see `unity-coding-standards`), so most assets show plain keys (`_spawnInterval:`), **not** `<Prop>k__BackingField`. The backing-field form only appears where `[field: SerializeField]` is used — rare in this codebase.
- **SO filename convention `%Name%SO.cs`;** keep `m_Name` matching the `.asset` filename (without extension).
- This skill is **data-editing only** — the runtime logging/architecture rules in `unity-coding-standards` do not apply to asset YAML.
