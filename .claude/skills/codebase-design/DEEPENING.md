...
> **In this project:** the live categories are **In-process** (pure ECS systems, math, formulas) and
> **Local-substitutable** (a Reflex service with a lightweight fake — `ISaveSystem` →
> `DummySaveSystem` is the canonical one). **Remote-but-owned** and **True-external** below rarely
> apply — this is a single-process Unity game with no owned network services and (today) no
> third-party SDK seams. Keep them in mind for when analytics / IAP / ads SDKs arrive.
...
new interface directly. No adapter needed. *(Most ECS systems, `MathHelper`, the `[Formula]`
evaluator, scorer math.)*

### 2. Local-substitutable

Dependencies that have local test stand-ins. Deepenable if the stand-in exists. The deepened module
is tested with the stand-in running in the test suite. The seam is internal; no port at the module's
external interface. *(The `ISaveSystem` → `DummySaveSystem` seam is the model to copy here.)*
...
Tests use an in-memory adapter. Production uses an HTTP/gRPC/queue adapter. *(Rare-to-absent in this
project.)*

### 4. True external (Mock)

Third-party services you don't control. The deepened module takes the external dependency as an
injected port; tests provide a mock adapter. *(None today; would apply to a future analytics/IAP/ads
SDK.)*
...
---
*Fork of mattpocock `codebase-design` (DEEPENING) — see ADR-0001.*
